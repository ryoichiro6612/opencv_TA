...
cv::Mat inpaint_mask;
cv::Mat original_image, whiteLined_image, inpainted, bitwise_not;

cv::Point prev_pt;
cv::Point rect_prev_pt;
...
void on_mouse(int event, int x , int y , int flags, void *){
  ...
  if(event == CV_EVENT_RBUTTONUP){
    cv::rectangle(inpaint_mask, rect_prev_pt, cv::Point(x, y), cv::Scalar(255), -1, 8, 0);
    cv::rectangle(whiteLined_image, rect_prev_pt, cv::Point(x, y), cv::Scalar::all(255), -1, 8, 0);
    cv::imshow("image", whiteLined_image);
    rect_prev_pt = cv::Point(-1, -1); // init the start point
  }
  else if(event == CV_EVENT_RBUTTONDOWN){
    rect_prev_pt = cv::Point(x, y); // set the start point
  }
  //*/
}
  ...
  while(loop_flag){
    ...
    switch(c){
      ...
      case 'b':
        ...
        cv::Mat temp;
        bitwise_not = original_image.clone();
        cv::bitwise_not(original_image, bitwise_not, inpaint_mask);
        cv::imshow("bitwise_not image", bitwise_not);
    }
  }
  return 0;
}
